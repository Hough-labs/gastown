...
func AgentEnv(cfg AgentEnvConfig) map[string]string {
	...
	if cfg.TownRoot != "" {
		// resolveDoltPort may return ErrDoltPortNotConfigured; that is not
		// fatal here — we fall through to process-env propagation below so
		// sessions with no config.yaml but a live GT_DOLT_PORT in the
		// environment still inject the right value (GH#2412).
		if port, err := resolveDoltPort(cfg.TownRoot); err == nil {
...
// ErrDoltPortNotConfigured is returned when no Dolt port can be resolved from
// any of the supported configuration sources. There is no compiled-in default —
// callers must fail loudly rather than silently falling back to a wrong port.
var ErrDoltPortNotConfigured = fmt.Errorf("dolt port not configured: set GT_DOLT_PORT, or run 'gt config set dolt.port <port>'")
...
//
// Returns ErrDoltPortNotConfigured if none of the above yield a valid port.
// There is intentionally no hardcoded default — wrong-port silent fallbacks
// cause split-brain and circuit-breaker storms, so unresolved callers must
...
func resolveDoltPort(townRoot string) (int, error) {
	...
	return 0, ErrDoltPortNotConfigured
}
